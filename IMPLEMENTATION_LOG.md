...
## Phase I 後の修正 — マップ操作と枝分かれ（2026-09-23）

- 原因: ReactFlow全体のdouble clickでQuick Addを開いており、＋／−ボタンの連打も拾っていた。空白pane自体のdouble clickだけを対象にし、double clickによる同時zoomも無効化。
- ＋／−、倍率表示、15〜250%スライダー、日本語の「全体表示」を常設。初回fitは最大85%。保存済みviewportはノードの寸法確定後に復元し、初期化中の保存による上書きを防止。
- 全体表示は上部・下部の操作UIを避けた余白を確保。Phoneの枝がノード操作バーに隠れないことをブラウザテストで確認。
- 子ノードはIDや文章の上書きではなく固定座標への重なりが原因。最新レコードと測定サイズを使い、同じトランザクション内で空き位置を選んで追加するよう修正。＋子 / Tab / 親のあるShift+Enterに適用。既存の手動配置は変更しない。
