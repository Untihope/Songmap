...
function MapContent({ projectId }: { projectId:string }) {
  ...
  return <div className="map-area" tabIndex={0} aria-label="Mind Map" onKeyDown={e=>{
    ...
    <ReactFlow<CanvasNode> nodes={canvasNodes} edges={canvasEdges} nodeTypes={types} onNodesChange={onNodesChange} fitView fitViewOptions={{padding:.4}} minZoom={.15} maxZoom={2.5}
      ...
      onNodeDragStop={(_,n,dragged)=>run(repository.atomic(async()=>{for(const moved of dragged.length?dragged:[n]) await repository.patch('nodes',moved.id,{position:moved.position})}).then(()=>setPositions({})))}
    ...
    {chosen&&<div className="node-actions"><button onClick={()=>add(chosen.id)}>＋子</button><button onClick={()=>setConnecting(chosen.id)}>接続</button><button aria-label="お気に入り" aria-pressed={chosen.favorite} onClick={()=>run(repository.patch('nodes',chosen.id,{favorite:!chosen.favorite}))}>{chosen.favorite?'★':'☆'}</button><button onClick={()=>useWorkspace.setState({editor:chosen.id})}>編集</button><button onClick={remove}>削除</button></div>}
