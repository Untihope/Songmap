...
export function App() { return <Routes><Route element={<Shell/>}><Route index element={<Home/>}/><Route path="songs" element={<Songs/>}/><Route path="songs/new" element={<NewSong/>}/><Route path="songs/:projectId" element={<Suspense fallback={<div className="page skeleton" aria-label="読み込み中"/>}><Workspace/></Suspense>}/><Route path="inbox" element={<Inbox/>}/><Route path="settings" element={<Foundation title="設定"/>}/><Route path="*" element={<div className="page"><h1>ページが見つかりません</h1><Link to="/">ホームへ戻る</Link></div>}/></Route></Routes> }
